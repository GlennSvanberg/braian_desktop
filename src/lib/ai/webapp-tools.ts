...
export function buildWorkspaceWebappTools(
  ...
  const publishTool = toolDefinition({
    ...
    description:
      'Run a production build of `.braian/webapp` with the correct base path and update the published app on **Dashboard → Apps**, and refresh `.braian/webapp-apps.json` (routes for the **Dashboard** tab). Use after meaningful UI changes when the user wants that view refreshed — not for every tiny edit. The user can also click **Publish** in Braian (Apps tab or Settings). Dev preview (hot reload) is under **Dashboard → Settings** or the App-mode artifact.',
