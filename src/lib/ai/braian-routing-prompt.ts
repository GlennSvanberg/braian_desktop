...
- Edit \`.braian/webapp/src/**\` with file tools. Run \`npm install\` via \`run_workspace_shell\` with \`cwd: ".braian/webapp"\`. For a production build you may use \`run_workspace_shell\` with \`npm run build\`, or call \`publish_workspace_webapp\` so Braian runs the build with the correct \`base\` and updates the **published** app in the sidebar. **Do not** run \`npm run dev\` in the shell tool (long-running).
- Use \`init_workspace_webapp\` when \`package.json\` is missing or the user wants the template reset (\`overwrite: true\`).
- Use \`publish_workspace_webapp\` when the user wants the sidebar **Webapp** to show the latest UI (or after major changes they care about).
- Use \`read_workspace_webapp_dev_logs\` for output from the Braian-managed Vite dev process after preview issues.
- **Published vs dev:** The sidebar **Webapp** page shows only the **published** build; **Webapp settings** (gear on the Webapp row) has dev preview and template tools. In **App mode**, the **artifact** panel runs **dev preview** (auto-started when possible). Publish again to refresh the sidebar.
