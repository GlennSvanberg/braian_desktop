...
/**
 * Eager tool when Make app is off: enables persisted app harness and tells the model
...
export function buildSwitchToAppBuilderTool(context: ChatTurnContext | undefined) {
  ...
  return toolDefinition({
    ...
    description: `Enable **workspace dashboard / in-app page** tools for this chat (Braian sidebar Dashboard and \`/dashboard/page/...\`). The user does not need to turn on "Make app" in the UI — you enable this by calling this tool.
