//! Shared stdio MCP helpers (spawn, JSON-RPC line, cwd). Spawn uses [`braian_mcp_core::stdio::spawn_stdio_server`] so PATH/shim logic stays in one place.
...
pub(crate) use braian_mcp_core::stdio::spawn_stdio_server;
