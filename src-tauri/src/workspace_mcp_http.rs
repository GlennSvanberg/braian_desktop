//! MCP Streamable HTTP client: JSON-RPC POST with optional SSE bodies and `Mcp-Session-Id`.
...
use crate::workspace_mcp_stdio::{
  mcp_tool_call_finalize_string, MCP_RPC_DEADLINE,
};
...
/// One-shot remote probe: initialize, `tools/list`, return summaries (no registry).
